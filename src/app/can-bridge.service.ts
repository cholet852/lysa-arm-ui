...
export class CanBridgeService{
  ...
  send(cmd: Command){
    ...
    const cur = {...this.states$.value}; const j = (cmd as any).joint;
    cur[j] = cur[j] ?? ({} as any); (cur[j] as any).joint = j;
    switch(cmd.type){
      case 'move':   (cur[j] as any).target = cmd.deg; (cur[j] as any).angle = cmd.deg; break;
      case 'speed':  (cur[j] as any).speed = cmd.v; break;
      case 'accel':  (cur[j] as any).accel = cmd.a; break;
      case 'current':(cur[j] as any).current_mA = cmd.mA; break;
      case 'micro':  (cur[j] as any).micro = cmd.u; break;
    }
...
function decodeMicro(code:number){ const t=[1,2,4,8,16,32,64,128,256]; return t[code] ?? 32; }
